...
static char vcid[] = "$Id: get_force_type.c,v 4.2 2000/05/16 21:57:54 vicadmin Exp $";
...
*************************************************************/
  ...
  ***************************************/
  ...
  if(strcasecmp("AIR_TEMP",optstr)==0){
    ...
    if(BINARY) {
      sscanf(cmdstr,"%*s %*s %s %lf %s %s",flgstr,
	     &param_set.TYPE[type].multiplier, param_set.TYPE[type].ncvarname, param_set.TYPE[type].ncfilename);
