  ...
  **************************************************/
  if(!(param_set.TYPE[VP].SUPPLIED && param_set.TYPE[SHORTWAVE].SUPPLIED)) {
    ...
    mtclim42_wrapper(0, 0, (theta_l-theta_s)*24./360., elevation, annual_prec,
		     ...
		     hourlyrad, SRADDT);
    for (i_hour=0; i_hour<24*Ndays; i_hour++) {
      if (isnan(vp[i_hour])) {
        for (j_hour=1; j_hour<24; j_hour++) {
          if (i_hour+j_hour<24*Ndays && !isnan(vp[i_hour+j_hour])) {
            vp[i_hour] = vp[i_hour+j_hour];
            break;
          }
          if (i_hour-j_hour>=0 && !isnan(vp[i_hour-j_hour])) {
            vp[i_hour] = vp[i_hour-j_hour];
            break;
          }
        }
      }
      if (hourlyrad[i_hour]<0.0001 || isnan(hourlyrad[i_hour])) hourlyrad[i_hour] = 0;
      //if (i_hour>365&&i_hour<420) printf("%g %g %g %g\n", hourlyrad[i_hour], vp[i_hour], prec[i_hour], tair[i_hour]);
    }
    //printf("%f %lf\n", 1/0, sqrt(-3.0));
