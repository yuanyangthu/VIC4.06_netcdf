...
void mtclim42_init(int have_dewpt, double elevation, double annual_prcp, 
		   ...
		   parameter_struct *p, data_struct *mtclim42_data, long SRADDT); 
...
void mtclim42_wrapper(int have_dewpt, int have_shortwave, double hour_offset,
		      ...
		      double *vp, double *hourlyrad, long SRADDT) 
{
  control_struct ctrl;
  parameter_struct p;
  ...
  if (ctrl.indewpt) {
    ...
    if (calc_srad_humidity(&ctrl, &p, &mtclim42_data, tiny_radfract, SRADDT)) {
