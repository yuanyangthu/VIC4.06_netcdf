  ...
  if(options.INIT_STATE) {
    ...
    if (options.BINARY_STATE_FILE) {
      if (options.STATE_GZIP)
        read_initial_model_state_binary_gzip(infiles.gzstatefile, prcp, global_param,
                                             Nveg, options.SNOW_BAND, cellnum, soil_con,
                                             Ndist, init_STILL_STORM, init_DRY_TIME);
      else
        read_initial_model_state_binary(infiles.statefile, prcp, global_param,
                                        Nveg, options.SNOW_BAND, cellnum, soil_con,
                                        Ndist, init_STILL_STORM, init_DRY_TIME);
    }
    else {
      if (options.STATE_GZIP)
        read_initial_model_state_ascii_gzip(infiles.gzstatefile, prcp, global_param,
                                            Nveg, options.SNOW_BAND, cellnum, soil_con,
                                            Ndist, init_STILL_STORM, init_DRY_TIME);
      else
        read_initial_model_state_ascii(infiles.statefile, prcp, global_param,
                                       Nveg, options.SNOW_BAND, cellnum, soil_con,
                                       Ndist, init_STILL_STORM, init_DRY_TIME);
    }
