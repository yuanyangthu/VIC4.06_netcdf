...
gzFile check_state_file_gzip(char                *init_state,
                             dmy_struct          *dmy,
...
*********************************************************************/
{
  ...
  if ( options.BINARY_STATE_FILE )
    gzstatefile = gzopen(init_state,"rb");
  else
    gzstatefile = gzopen(init_state,"r");
  ...
  *startrec = 0;
  ...
  if ( options.BINARY_STATE_FILE ) {
    gzread( gzstatefile, (voidp *) &startyear, sizeof(int) );
    gzread( gzstatefile, (voidp *) &startmonth, sizeof(int) );
    gzread( gzstatefile, (voidp *) &startday, sizeof(int) );
  }
  else {
    gzgets(gzstatefile, buff, MAXSTRING);
    sscanf(buff,"%i %i %i\n", &startyear, &startmonth, &startday );
  }

  /* Check simulation options */
  if ( options.BINARY_STATE_FILE ) {
    gzread( gzstatefile, (voidp *) &tmp_Nlayer, sizeof(int) );
    gzread( gzstatefile, (voidp *) &tmp_Nnodes, sizeof(int) );
  }
  else {
    gzgets(gzstatefile, buff, MAXSTRING);
