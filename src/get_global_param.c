...
**********************************************************************/
{
  ...
  strcpy(global.statefreq, "NONE");
  
  global.lat = (double *) calloc(MAX_CELLS, sizeof(double));
  global.lon = (double *) calloc(MAX_CELLS, sizeof(double));
  ...
  options.OUTPUT_GRID_DX = MISSING;
  options.STATE_GZIP      = FALSE;
  options.TRUE_LATLON     = FALSE;
  options.YREV            = FALSE;
  ...
  while(!feof(gp)) {
	...
	******************/
	...
	sscanf(cmdstr,"%*s %f",&options.MIN_WIND_SPEED);
	  ...
	  strcpy(names->init_state,flgstr);
        ...
        sscanf(cmdstr,"%*s %s %s",global.statename,global.statefreq);
      }
	...
	**********************************/
	...
	if (strcasecmp(flgstr, "BINARY") == 0)
	  ...
	  param_set.FORCE_FORMAT[file_num] = ASCII;
        else if (strcasecmp(flgstr, "NCFLOAT") == 0)
          param_set.FORCE_FORMAT[file_num] = NCFLOAT;
        else if (strcasecmp(flgstr, "NCDOUBLE") == 0)
          param_set.FORCE_FORMAT[file_num] = NCDOUBLE;
	else
	  nrerror("FORCE_FORMAT must be either ASCII, BINARY, NCFLOAT, or NCDOUBLE.");
      }
	...
	****************************/
      ...
      else if(strcasecmp("STATE_GZIP",optstr)==0) {
        sscanf(cmdstr,"%*s %s",flgstr);
        if(strcasecmp("TRUE",flgstr)==0) options.STATE_GZIP = TRUE;
        else options.STATE_GZIP = FALSE;
      }
      
      else if(strcasecmp("TRUE_LATLON",optstr)==0) {
        sscanf(cmdstr,"%*s %s",flgstr);
        if(strcasecmp("FALSE",flgstr)==0) options.TRUE_LATLON = FALSE;
        else {
          options.TRUE_LATLON = TRUE;
          strcpy(names->true_latlon,flgstr);
        }
      }

      else if(strcasecmp("YREV",optstr)==0) {
        sscanf(cmdstr,"%*s %s",flgstr);
        if(strcasecmp("TRUE",flgstr)==0) options.YREV = TRUE;
        else options.YREV = FALSE;
      }
