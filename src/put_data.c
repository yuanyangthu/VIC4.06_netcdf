  ...
  if(rec >= skipyear) {
    if (options.GRID_OUTPUT) {
        if (options.BINARY_OUTPUT)
            write_data_nldas_grid(out_data, (filenames_struct *)NULL, (global_param_struct *)NULL, cell_cnt, -9999, rec);
        else
            write_data_netcdf(out_data, (filenames_struct *)NULL, (global_param_struct *)NULL, cell_cnt, -9999, rec);
    }
