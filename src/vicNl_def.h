...
#define NCFLOAT 3               /* met file format flag */
#define NCDOUBLE 4              /* met file format flag */
...
#define UNDEF           -999.9
#define MAX_CELLS     83888608           /* enough for 5km global simulation */
#define MAX_STATES          12           /* maximum number of dates to store/dump model states, requiring significant additional memory */
...
typedef struct {
  ...
  char  true_latlon[MAXSTRING]; /* name of true lat/lon file if the grid is not on a rectilinear lat/lon projection */
} filenames_struct;
...
typedef struct {
  ...
  char   STATE_GZIP;            /* TRUE: both input & output statefiles gzipped */
  char   TRUE_LATLON;           /* TRUE: grid is projected on a non-lat/lon grid with true lat/lon provided in a file specified hereafter */
  char   YREV;                  /* TRUE: latitude written from top down (default is from bottom up) */
...
typedef struct {
  ...
  int    ncells;     /* number of cells in excution */
  double *lat;       /* cell latitude */
  double *lon;       /* cell longitude */
  
  char   statefreq[MAXSTRING]; /* state file dumping frequency: ONCE/YEAR/MONTH/DAY */
} global_param_struct;
